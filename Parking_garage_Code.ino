...
int slot2 = 6;   //sLot2 pin on digital 4 
...
void setup() 
{
gate.attach(9);     //connecting the gate servo on pin 5
...
if(   !(digitalRead(gateSensor)) && digitalRead(slot1) && digitalRead(slot2))   //slot1 & slot2 empty
 ...
 if( !(digitalRead(gateSensor)) && !(digitalRead(slot1)) && digitalRead(slot2))  //car on slot1,slot2 free
  {
      Serial.println("Welcome, Available: sLot1");  // slot1 available
   ...
   {
     Serial.println("Welcome, Available: sLOT2");  // slot2 available
      digitalWrite(slot1_l,HIGH);
      digitalWrite(slot2_l,LOW);
      delay(1000);
     digitalWrite(gate_grn,HIGH);
     gate.write(75); 
      delay(3000);                //gate will open after the dealy of 1 second
   }
